...
public class UserInfoService {
    ...
    public UserInfoResponse update(UserInfoRequest userInfoRequest) throws IOException {
        final var userInfoToQuery = new UserInfo(userInfoRequest.getUsername(), null, null, null, null, null);
        ...
        userInfoMapper.update(new UserInfo(userInfoRequest.getUsername(), userInfoRequest.getNickname(), userInfoRequest.getPersonalSign(), avatar, previousUserInfo.getMyPosts(), previousUserInfo.getMyCollections()));
    ...
    public UserInfoResponse query(UserInfoRequest userInfoRequest) {
        var userInfoRes = userInfoMapper.query(new UserInfo(userInfoRequest.getUsername(), null, null, null, null, null));
        if (userInfoRes == null) {
            return new UserInfoResponse(false, UserInfoResponseFailedReason.USERNAME_DOES_NOT_EXIST, null);
        }
